\subsection{Zeeman splitting of $^{87} {\rm Rb}$ hyperfine ground states}
The angular momentum of $^{87} {\rm Rb}$ interacts with external magnetic field and the hyperfine states split into sub-states. Here we use perturbation theory to calculate the energy of each sub-states of $^{87} {\rm Rb}$ ground state.
...
\end{equation}
Treat the interaction with external magnetic field as a perturbation, the first order perturbed energy for state $\ket{F,m_F}$ is
\begin{equation}
...
The energy of $\ket{F,m_F}$ states are listed here, in the units of ${\rm MHz}$, ${\rm MHz/G}$ and ${\rm MHz/G^2}$. The numbers are useful for quick estimation of magnetic field in the lab.
\begin{align}
...
The second order perturbation leads to quadratic Zeeman shift which has sizable effect when magnetic field on the order of $ 10{\rm G}$. In our experiments, the quadratic Zeeman shift makes the energy difference between $\ket{1,0}$ and $\ket{1,-1}$ large enough than the differece between $\ket{1,0}$ and $\ket{1,1}$. So we can effectly treat the $F=1$ manfold as a two level system by decoupling $\ket{1,-1}$. 
...
\end{align}
The energy of states $\ket{F,m_F}$ is shown in Fig.~(\ref{fig:BField}) for magnetic field up to 15000 {\rm G}.
