...
\end{align}
here, $\gamma_{21}$ is the decay rate of coherence and $R$ is optical pumping rate, the rate the atoms are pumped from the ground state to the excited state. When the field is weak,
\begin{equation}
    |\Omega| \ll \gamma
\end{equation}
...
\end{equation}
The potential is spin independent, its strength is propotional to the intensity of the field $|\Omega\Vec{r}|^2$ and the sign of $\delta$ determines if the potential is repulsive or attractive. When $\delta > 0$, the field is red detuned, and the potential is lower for stronger intensity, so the potential is attractive. When the field is blue detuned, it is repulsive.  

This potential is known as the dipole potential, it originates from the AC stark shift. Due to the large detuning, it does not drive transition of internal states but changes the energy of the ground state so it is effectively a spin independent potential. Dipole potential is widely used in atomic physics experiments, when $\delta$ is large, it is valid for strong field. So it can be used to make attractive trap, called dipole trap, to trap atoms, do evaporative cooling and make optical lattice. 
Repulsive potential is also very useful, it cal be used to make 1d trap when the laser is in Laguerre-Gaussian mode \cite{salces2018equations} and make random repulsive potential (optical speckle) which is discussed in detail in Chpt.().
