...
\end{aligned}
\end{equation}
to characterize the statistical properties of the disordered electric field.  Equation~\eqref{eq:zeromean} implies that the second term is zero.  At $z=0$, the uniform phase distribution implies $\langle E^+({\bf r}; 0) \rangle = 0$, giving
\begin{align*}
...
\end{align}
take the same form as a usual Gaussian beams focused at $z_0$.  Lastly, as in Fig.~\ref{fig:speckle1}(b), an ideal lens with focal length $f$ at position $z_L$ gives new Gaussian beam parameters defined by
...
\end{align}
simply related to the field-field correlation function in Eq.~(\ref{eq:C_E}), contains no information about the optical speckle excepting for the changed $M^2$.
