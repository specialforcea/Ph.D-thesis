...
For a two-particle wave function $\ket{\phi_1\phi_2}$, if the two particles are fermions, they can not occupy the same state. $\ket{\phi_1\phi_2} = -\ket{\phi_2\phi_1}$, if $\phi_1 = \phi_2$, $\ket{\phi_1\phi_2} = 0$. For Bosons, this is not the case. Any number of Bosons can occupy the same state and the system has the lowest energy when all the Bosons are in the ground state.
