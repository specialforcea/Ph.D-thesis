...
The enhanced transport described here results from the same physics giving rise to a spin transistor in Ref.~\cite{Mossman2019}, which also relied on a combination if kinematic and matrix-element effects to yield non-reciprocal appearing transport behavior.
In the appendix, we describe an explicit experimental proposal using laser speckle derived from $532\ {\rm nm}$ green laser and an off the shelf optical diffuser.
...
The protection from back scattering is independent of quantum statistics: non-interacting fermions would experience a conductivity increased by the factor predicted by the FGR when the Fermi energy resides in the SOC gap.
